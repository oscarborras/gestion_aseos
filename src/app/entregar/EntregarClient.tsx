...
export default function EntregarClient({
    ...
    // Lógica para encontrar al SIGUIENTE alumno al que se le puede dar llave
    // Debe haber un aseo disponible para su sexo
    const findNextEligible = () => {
        for (const student of waitingList) {
            ...
            if (matchingAseo) {
                return { student, aseo: matchingAseo }
            }
        }
        return null
    }

    const nextAssignment = findNextEligible()
    ...
    const handleDeliver = async (waitingId: number, alumnoId: string, aseoId: number) => {
        setLoading(true)
        const result = await entregarTurno(waitingId, alumnoId, aseoId)
        setLoading(false)

        if (result.error) {
            toast.error(result.error)
        } else {
            toast.success('Llave entregada con éxito')
            router.refresh()
        }
    }
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-100 dark:border-slate-800 shadow-xl relative overflow-hidden h-full">
                            ...
                            <div className="space-y-6">
                                <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700/50 space-y-4">
                                    <div className="flex items-start gap-4">
                                        <div className="w-10 h-10 bg-white dark:bg-slate-700 rounded-full flex items-center justify-center border border-slate-100 dark:border-slate-600">
                                            <User className="w-5 h-5 text-slate-400" />
                                        </div>
                                        <div>
                                            <p className="text-xs font-bold text-slate-400 uppercase">Alumno/a</p>
                                            <p className="font-bold text-slate-900 dark:text-white text-lg leading-tight">{nextAssignment.student.nombre}</p>
                                            <p className="text-sm text-slate-500">{nextAssignment.student.unidad}</p>
                                        </div>
                                    </div>

                                    <div className="flex items-start gap-4 pt-2">
                                        <div className="w-10 h-10 bg-primary-brand/10 rounded-full flex items-center justify-center border border-primary-brand/20">
                                            <CheckCircle className="w-5 h-5 text-primary-brand" />
                                        </div>
                                        <div>
                                            <p className="text-xs font-bold text-slate-400 uppercase">Aseo Destino</p>
                                            <p className="font-bold text-slate-900 dark:text-white text-lg leading-tight">{nextAssignment.aseo.nombre}</p>
                                        </div>
                                    </div>
                                </div>

                                <button
                                    onClick={() => handleDeliver(nextAssignment.student.id, nextAssignment.student.alumno_id, nextAssignment.aseo.id)}
                                    disabled={loading}
